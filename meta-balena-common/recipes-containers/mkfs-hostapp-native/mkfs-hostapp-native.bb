...
do_compile () {
    ...
    IMAGETAG="${PN}:$(date +%s)"
    DOCKER_API_VERSION=1.24 docker build --tag ${IMAGETAG} ${B}/work
    DOCKER_API_VERSION=1.24 docker save "$IMAGETAG" > ${B}/work/mkfs-hostapp-image.tar
    DOCKER_API_VERSION=1.24 docker rmi "$IMAGETAG"
